...
public:
    ...
    {
		ANT_ASSERT(! (this == & other));	// don't copy over self!
		if (m_bValid)
	...
	T const & operator * () const			{ ANT_ASSERT(m_bValid); return * GetT(); }
	T & operator * ()						{ ANT_ASSERT(m_bValid); return * GetT(); }
	T const * const operator -> () const	{ ANT_ASSERT(m_bValid); return GetT(); }
	T		* const operator -> ()			{ ANT_ASSERT(m_bValid); return GetT(); }
